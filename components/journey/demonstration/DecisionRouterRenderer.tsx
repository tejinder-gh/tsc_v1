...
interface DecisionRouterRendererProps {
  scenario: DemonstrationScenario;
  onStarted?: () => void;
  onCompleted?: () => void;
}

export function DecisionRouterRenderer({
  scenario,
  onStarted,
  onCompleted,
}: DecisionRouterRendererProps) {
      ...
      <div className="rounded-[4px] border border-[var(--tsc-line)] bg-white p-5 sm:p-6 lg:p-7 shadow-[0_2px_12px_rgba(18,19,15,0.03)]">
            ...
            <span
              className={`inline-block h-2 w-2 rounded-full ${
                scenario.isException
                  ...
                  : "bg-[var(--tsc-action)] ring-4 ring-[var(--tsc-action)]/20"
              }`}
              aria-hidden="true"
            />
            ...
            return (
              <div
                ...
                className={`relative rounded-[3px] border transition-all duration-200 p-4 ${
                  isCurrent
                    ? "border-[var(--tsc-action)] bg-[var(--tsc-surface)] ring-1 ring-[var(--tsc-action)]/20"
                    : isCompleted
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  {step.statusTag && isCompleted && (
                    <span
                      className={`font-mono text-[10px] font-semibold tracking-wider uppercase px-1.5 py-0.5 rounded-[2px] border ${
                        step.phase === "EXCEPTION"
                          ? "border-amber-600/40 text-amber-800 bg-amber-50"
                          : "border-[var(--tsc-line)] text-[var(--tsc-muted)] bg-white"
                      }`}
                    >
                      {step.statusTag}
                    </span>
                ...
                {step.dataPayload && isCompleted && (
                  <div className="mt-3 rounded-[3px] border border-[var(--tsc-line)] bg-[var(--tsc-ink)] p-3 text-white font-mono text-[11px] leading-relaxed overflow-x-auto">
                    <div className="text-[10px] text-white/50 font-semibold tracking-wider uppercase mb-1.5">
                      DECISION CRITERIA & STATE
                    </div>
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-t border-[var(--tsc-line)] pt-4">
          <div className="flex items-center gap-3">
            ...
            >
              {isRunning ? "Evaluating..." : "Run all steps"}
            </button>
