...
| :--- | :--- | :--- | :--- | :--- |
| `/api/lead` | `POST` | [`app/api/lead/route.ts`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/app/api/lead/route.ts) | Honeypot check + Zod validation + 32KB payload limit | Validates inbound lead submissions from forms and interactive demonstration architecture requests (including typed `journey_context`). Delivers to `LEAD_WEBHOOK_URL` (Zapier, Make, n8n) with timeouts and error shielding, stripping unrecognised client fields. |
