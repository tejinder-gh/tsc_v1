...
| :--- | :--- | :--- | :--- |
| `/` | [`app/(marketing)/page.tsx`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/app/(marketing)/page.tsx) | Public | **Homepage**: Editorial Studio narrative (Global Masthead, 01 Intent Router & Live Observed System, 02 How We Decide, 03 System Studies, 04 Founder POV, 05 Curated Explore, 06 Open Prompt, Minimal Footer). |
...
| `/dashboard/workflows` | [`app/dashboard/workflows/page.tsx`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/app/dashboard/workflows/page.tsx) | Operator Auth (Clerk) | **Feature Workflows & Trigger Console**: Manually trigger and observe scheduler ticks, inbound SMS simulation, lead webhook pipeline, hardware SMS relay HMAC ingest, Second Brain RAG & IAM registry, catalog integrity audits, and live telemetry matrix. |
...
### 1.4 Operator Authentication Routes
...
### 1.5 Internal Review & Developer Tools
...
| :--- | :--- | :--- | :--- | :--- |
| `/api/lead` | `POST` | [`app/api/lead/route.ts`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/app/api/lead/route.ts) | Rate limited (10 req/min) + Honeypot check + Zod validation + 32KB payload limit | Validates inbound lead submissions from forms and interactive demonstration architecture requests (including typed `journey_context`). Throttles bot abuse with 429 Retry-After without logging PII. Delivers to `LEAD_WEBHOOK_URL` (Zapier, Make, n8n) with timeouts and error shielding, stripping unrecognised client fields. |
| `/api/newsletter/subscribe` | `POST` | [`app/api/newsletter/subscribe/route.ts`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/app/api/newsletter/subscribe/route.ts) | Rate limited (10 req/min) + Honeypot drop + Zod validation + 32KB payload limit | Registers newsletter subscriptions with truthful delivery semantics: validates publication slugs, throttles abusive submissions with 429 Retry-After, enforces 503 fail-closed in production if webhook URL is absent, returns 502 on upstream timeout/failure, emits PII-scrubbed audit logs with stable reason codes, and returns success only on 2xx webhook delivery. |
...
| `/api/v1/relay` | `POST` | [`app/api/v1/relay/route.ts`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/app/api/v1/relay/route.ts) | HMAC-SHA256 signature + distributed atomic nonce replay protection (300s TTL) | Hardware SMS relay gateway for Android SMS capture devices. Validates hardware cryptographic signatures, rejects replayed nonces atomically via Postgres check-and-set, sanitizes email headers/subjects, and forwards to delivery adapters with zero message-body persistence. |
| `/api/catalogue` (alias: `/api/catalog`) | `GET` | [`app/api/catalogue/route.ts`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/app/api/catalogue/route.ts) | Database machine key auth (`Authorization: Bearer <key>`, `x-api-key`, or `?key=`) matching `second_brain_security.authenticate_agent` | Self-documenting public endpoint & route catalog. Authenticates machine identities against database credentials, exposing all public UI routes, dynamic hubs, public APIs, machine-readable documents, and permanent redirects. Supports `?type=`, `?expand=slugs`, and `?format=openapi`. |

---

## 3. Internal Agent IAM & Automation OS APIs (`/api/internal/v1/...`)

All internal endpoints require authentication via Bearer token (`Authorization: Bearer <key_id>.<secret>`) or `x-api-key`, enforced by the Second Brain database-native IAM system ([`authenticateAgent`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/lib/second-brain/auth/authenticate.ts) and [`authorize`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/lib/second-brain/auth/authorize.ts)). All endpoints utilize the standardized [`withAgentApi`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/lib/second-brain/auth/withAgentApi.ts) wrapper with strict `.strict()` Zod schema validation and error shielding.
...
| `/api/internal/v1/context/search` | `POST` | [`app/api/internal/v1/context/search/route.ts`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/app/api/internal/v1/context/search/route.ts) | `context.read` | Canonical context & RAG engine. Validated with strict schema. Resolves high-level domains and keywords to authoritative Notion/Drive documents, playbooks, and guidelines. |
...
| `/api/internal/v1/automations/occurrences/ensure` | `POST` | [`app/api/internal/v1/automations/occurrences/ensure/route.ts`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/app/api/internal/v1/automations/occurrences/ensure/route.ts) | `automation.occurrence.write` | Validated with strict schema. Idempotently creates or returns an occurrence slot record for a scheduled job run. |
| `/api/internal/v1/automations/claims/acquire` | `POST` | [`app/api/internal/v1/automations/claims/acquire/route.ts`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/app/api/internal/v1/automations/claims/acquire/route.ts) | `automation.claim.acquire` | Validated with strict schema. Atomically acquires a timed lease claim on an occurrence to prevent duplicate concurrent runs. |
| `/api/internal/v1/automations/claims/release` | `POST` | [`app/api/internal/v1/automations/claims/release/route.ts`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/app/api/internal/v1/automations/claims/release/route.ts) | `automation.claim.release` | Validated with strict schema. Releases an active execution lease claim when execution finishes or aborts. |
| `/api/internal/v1/automations/attempts/start` | `POST` | [`app/api/internal/v1/automations/attempts/start/route.ts`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/app/api/internal/v1/automations/attempts/start/route.ts) | `automation.attempt.write` | Validated with strict schema. Logs the start timestamp and sequence number of a job execution attempt. |
| `/api/internal/v1/automations/attempts/finish` | `POST` | [`app/api/internal/v1/automations/attempts/finish/route.ts`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/app/api/internal/v1/automations/attempts/finish/route.ts) | `automation.attempt.write` | Validated with strict schema. Records completion status (`succeeded`, `failed`, `deferred`), scheduler disposition, and execution duration. |
| `/api/internal/v1/automations/cycles/upsert` | `POST` | [`app/api/internal/v1/automations/cycles/upsert/route.ts`](file:///Users/tejindersingh/dev/projects/TheSkillCorner/app/api/internal/v1/automations/cycles/upsert/route.ts) | `automation.cycle.write` | Validated with strict schema. Upserts runner batch cycle summaries, jobs evaluated, and batch metrics without raw body pass-through. |
